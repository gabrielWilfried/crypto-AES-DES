# Define the S-Box in hexadecimal format as a string and remove spaces
...
# Convert the S-Box string into a bytearray
s_box = bytearray.fromhex(s_box_string)

# Define the sub_word function to perform the substitution using the S-Box
def sub_word(word: [int]) -> bytes:
    substituted_word = bytes(s_box[i] for i in word)
    return substituted_word

# Define the rcon function to calculate the round constant
...
# Define the xor_bytes function to perform byte-wise XOR operation
def xor_bytes(a: bytes, b: bytes) -> bytes:
    return bytes([x ^ y for (x, y) in zip(a, b)])

# Define the rot_word function to perform the word rotation
def rot_word(word: [int]) -> [int]:
    return word[1:] + word[:1]

# Define the key_expansion function to expand the key for AES
def key_expansion(key: bytes, nb: int = 4) -> [[[int]]]:
    ...
    # (This function handles key expansion, but you might want to add comments for the code inside it as well.)

# Define the add_round_key function to perform the AddRoundKey step
...
# Define the sub_bytes function to perform the SubBytes step
def sub_bytes(state: [[int]]):
    for r in range(len(state)):
        state[r] = [s_box[state[r][c]] for c in range(len(state[0]))]

# Define the shift_rows function to perform the ShiftRows step
...
# Define the xtime function to perform the xtime operation for AES
def xtime(a: int) -> int:
    if a & 0x80:
        return ((a << 1) ^ 0x1b) & 0xff
    return a << 1

# Define the mix_column function to perform the MixColumns step
...
# Define the mix_columns function to perform the MixColumns step on the entire state
def mix_columns(state: [[int]]):
    for r in state:
        mix_column(r)

# Define the state_from_bytes function to convert a byte array into the state format
...
# Define the aes_encryption function to perform AES encryption
...
# Define the inv_shift_rows function to perform the inverse ShiftRows step
...
# Define the inv_s_box and related functions for the inverse S-Box
# (This part is for the inverse S-Box, similar to the S-Box, and you can add comments for the functions as well.)
...
# Define the inv_mix_column function to perform the inverse MixColumns step on a single column
